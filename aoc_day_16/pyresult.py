...
INPUT_PATH = Path(SCRIPT_PATH, "in.txt")
...
def parse_input(data):
    ...
    hrz_sym = [
        [sym.start() for sym in re.finditer(r"[^\.]", line)] for line in data_list
    ]
    vert_sym = [
        [sym.start() for sym in re.finditer(r"[^\.]", "".join(line))]
        for line in zip(*data_list)
    ]  # Use zip to get columns
    bool_map = [[False for _ in range(len(line))] for line in data_list]
    for idx, line in enumerate(
        hrz_sym
    ):  # After creating boolean map, re-add the symbols into the map
        for sym in line:
...
def get_symbol_value(direction, a, m):
    ...
    match direction:
        case "R" | "D":
            return iter([idx for idx in m[a[0]] if idx >= a[1]])
        case "L" | "U":
            return iter(reversed([idx for idx in m[a[0]] if idx <= a[1]]))
...
def light_beam(point, m, bool_map, direction):
    ...
    if value is not None:
        ...
        if (
            direction in "LR"
        ):  # Use the redirect map to determine where to send the light beam next based on direction
        ...
        for i in get_range_edge(
            direction, point[1], len(bool_map[point[0]])
        ):  # You've hit an edge, let the program know to close this branch
            if direction in "LR":
                bool_map[point[0]][i] = True
            else:
                bool_map[i][point[0]] = True
...
def move(l, h, v, curr_point, curr_dir, mirror_hit):
    ...
    match curr_dir:  # Each direction is slightly different, so determine that first
        case "R":
            curr_point, l, new_dir = light_beam(
                (curr_point[0], (curr_point[1] + 1)), h, l, curr_dir
            )
        case "L":
            curr_point, l, new_dir = light_beam(
                (curr_point[0], (curr_point[1] - 1)), h, l, curr_dir
        ...
        case "D":
            curr_point, l, new_dir = light_beam(
                (curr_point[1], (curr_point[0] + 1)), v, l, curr_dir
            )
        case "U":
            curr_point, l, new_dir = light_beam(
                (curr_point[1], (curr_point[0] - 1)), v, l, curr_dir
    ...
    if curr_point is not None:  # If curr_point is None, you've hit a wall
        if curr_dir not in mirror_hit.setdefault(
            curr_point, []
        ):  # If curr_dir is in mirror_hit(curr_point), you've already hit that mirrow from this direction (loop)
...
def get_energized_tiles(bool_map, mirror_hit):
    ...
    return len(mirror_hit) + sum(
        1 for line in bool_map for item in line if item and isinstance(item, bool)
    )
...
def check_all_options(data):
    ...
    for i in range(0, length):
        directions = [
            ...
            (-1, i, "D"),
            (length, i, "U"),
        ]  # These are grouped into x value, y value, and direction
        for entry in directions:
            l, h, v = copy.deepcopy(
                data
            )  # Make a copy since you are going to need a fresh copy each time
            l, mirror_hit = move(
                l, h, v, (entry[0], entry[1]), entry[2], {}
            )  # Run the recursive move function for each direction for value i
            curr_total_tiles = get_energized_tiles(l, mirror_hit)
            print(
                f"Entry point ({entry[0]}, {entry[1]}) has {curr_total_tiles} energized tiles"
            )
